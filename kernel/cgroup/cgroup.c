					      ...
					      struct cgroup_subsys *ss)
{
	...
	if(ss->id == cpuset_cgrp_id) {
		...
		pr_warn("%s: cgroup=%s cgid=%llu cgroup_subsys=%s ssid=%d cpuset_cgrp_id=%d", __func__, name, (unsigned long long)cgroup_id(cgrp), ss->name, ss->id, cpuset_cgrp_id);
		cpuset_add_cg((unsigned long long)cgroup_id(cgrp), name);
	}
#endif
